...
export const popular_movie_slice = createAsyncThunk("popular_movie_slice", async (thunkAPI) => {
    try {
        const options = {
            method: 'GET',
            headers: {
...
export const popularMovieSlice = createSlice({
    ...
    extraReducers: (builder) => {
        builder
            .addCase(popular_movie_slice.pending, (state) => {
                state.loading = true
            })
            .addCase(popular_movie_slice.fulfilled, (state, action) => {
                state.data = action.payload
                state.isSuccess = true
                state.loading = false
            })
            .addCase(popular_movie_slice.rejected, (state, action) => {
